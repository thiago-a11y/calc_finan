...
**Versão Atual:** v0.58.9 (02/Abr/2026)
...
- **v0.58.9** — **Guard de Loading no Mission Control** — guard `carregando` no topo do componente que mostra spinner ate autenticacao estar pronta, impedindo useEffects de executar com estado invalido
- **v0.58.8** — **Correção FINAL Crash Mission Control ao Criar Sessão** — hasToken guard em todas API calls; reset estado completo antes de navegar em criarSessao; JSON parse defensivo; null checks em todas propriedades; polling useEffects com hasToken guard
